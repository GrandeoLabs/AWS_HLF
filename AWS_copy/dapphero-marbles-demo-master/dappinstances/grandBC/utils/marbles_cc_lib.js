//This library is re-done for G-BC and all the functions from index.js has been imported here-------------------------------------------------------------------
...
module.exports = function (enrollObj, g_options, fcw, logger) {
	...
	ck_chaincode.check_if_already_instantiated = function (options, cb) {
		console.log('');
	...
	ck_chaincode.check_version = function (options, cb) {
		console.log('');
...
ck_chaincode.get_all_hashes = function(options, cb) {
    console.log('')
    logger.info('Getting all hashes ')
    var opts = {
        peer_urls: g_options.peer_urls,
        peer_tls_opts:g_options.peer.peer_tls_opts,
        channel_id:g_options.channel_id,
        chaincode_id:g_options.chaincode_id,
        chaincode_version: g_options.chaincode_version,
        event_urls:g_options.event_urls,
        endorsed_hook:g_options.endorsed_hook,
        ordered_hook:g_options.ordered_hook,
        cc_function:'listChainKeys',
        cc_args: [],
    }
}
...
fcw.query_CK = function(options, cb){
    logger.info('Fetching CK information')
    var opts = {
        peer_urls:g_options.peer_urls,
        peer_tls_opts: g_options.peer_tls_opts,
        channel_id:g_options.channel_id,
        chaincode_version:g_options.chaincode_version,
        chaincode_id:g_options.chaincode_id,
        cc_function:'queryChainKey',
        cc_args:[options.args.ck_id]
    }
    fcw.query_chaincode(enrollObj,opts,cb)

}
fcw.record_hashes = function(options, cb) {
    logger.info('Recording Hashes...')
    var opts = {
        peer_urls: g_options.peer_urls,
        peer_tls_opts: g_options.peer_tls_opts,
        channel_id: g_options.channel_id,
        chaincode_id: g_options.chaincode_id,
        chaincode_version: g_options.chaincode_version,
        cc_function: 'recordChainKey',
        cc_args:[options.args.documentID, options.args.glue,options.args.typeOfDocument]
        }
        fcw.query_chaincode(enrollObj, opts, cb);
}
return ck_chaincode;
